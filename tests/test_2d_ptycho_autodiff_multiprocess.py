...
torchrun --nnodes=1 --nproc_per_node=2 test_2d_ptycho_autodiff_multiprocess.py
"""
...
class Test2dPtychoAutodiffMultiprocess(tutils.TungstenDataTester):
    
    @pytest.mark.local
    @tutils.TungstenDataTester.wrap_recon_tester(name='test_2d_ptycho_autodiff_multiprocess')
    def test_2d_ptycho_autodiff_multiprocess(self):        
...
if __name__ == '__main__':
    ...
    tester = Test2dPtychoAutodiffMultiprocess()
    tester.setup_method(name="", generate_data=False, generate_gold=args.generate_gold, debug=True)
    tester.test_2d_ptycho_autodiff_multiprocess()
