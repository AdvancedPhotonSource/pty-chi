...
class TestPtychoTiming(tutils.TungstenDataTester):
    ...
    def test_2d_ptycho_lsqml_opr_timing(self):        
        ...
        options.reconstructor_options.num_epochs = 30
        options.reconstructor_options.allow_nondeterministic_algorithms = False
        
        task = PtychographyTask(options)
        
        # First, run a few epochs for torch.compile to compile the kernels
        task.run(5)
        
        timer_utils.toggle_timer(True)
        task.run(options.reconstructor_options.num_epochs)
        
        timer_utils.plot_elapsed_time_bar_plot()
...
if __name__ == '__main__':
    ...
    tester = TestPtychoTiming()
    tester.setup_method(name="", generate_data=False, generate_gold=args.generate_gold, debug=True)
    tester.test_2d_ptycho_lsqml_opr_timing()
