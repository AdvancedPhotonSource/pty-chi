...
class TestMultislicePtychoLSQMLNonsqPsize(tutils.BaseTester):

    @tutils.BaseTester.wrap_recon_tester(name='test_multislice_ptycho_lsqml_nonsq_psize')
    def test_multislice_ptycho_lsqml_nonsq_psize(self):
        self.setup_ptychi(cpu_only=False)
        
        data, probe, pixel_size_m, positions_px = self.load_data_ptychodus(
            *self.get_default_input_data_file_paths('multislice_ptycho_AuNi_nonsq_psize'),
            subtract_position_mean=True
        )
        pixel_width_m = pixel_size_m
        positions_px[:, 0] = positions_px[:, 0] / 2
        ...
        options.object_options.pixel_size_m = pixel_width_m
        options.object_options.pixel_size_aspect_ratio = 0.5
...
if __name__ == '__main__':
    ...
    tester = TestMultislicePtychoLSQMLNonsqPsize()
    tester.setup_method(name="", generate_data=False, generate_gold=args.generate_gold, debug=True)
    tester.test_multislice_ptycho_lsqml_nonsq_psize()
